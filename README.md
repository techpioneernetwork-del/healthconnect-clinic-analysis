...
Understand the appointment dataset, assess its quality, calculate KPIs, run exploratory analysis on no-show drivers, validate findings statistically, and translate results into evidence-based business recommendations — feeding relevant findings to the Data Science and Project Management tracks along the way.
...
Problem Understanding (Week 4) → Analysis & Initial Implementation (Week 5) → **Integration & Validation (Week 6, current)** → Testing & Refinement (Week 7) → Final Presentation (Week 8)

## Tools & Skills Applied

Python (pandas, scipy, scikit-learn) · Statistical Hypothesis Testing (chi-square, likelihood-ratio test) · Logistic Regression · Data Quality Assessment · KPI Design & Validation · Cross-Track Collaboration · Healthcare Data Analysis
